function param_array = mcmc_up( ...
    N_iter, N_init, N_burn, init_param, data, ...
    beta0, B0, a0, b0, c0, C0, alpha0, Alpha0 ...
    )
    ...
    for n = 2:N_init
        ...
        param = sample_sigma_sq(param, data, a0, b0, beta0, B0);
...
function loglikelihood = log_posterior(param, data)
    ...
    loglikelihood = - N * (T-1)/2 * log(sigma_sq) + logdetS - HH_sum;
end
...
function param = sample_sigma_sq(param, data, a0, b0, beta0, B0)
    ...
    an = a0 + N * (T-1) + K;
    bn = b0 + ((beta - beta0)' * (B0 \ (beta - beta0))) + HH_sum;
