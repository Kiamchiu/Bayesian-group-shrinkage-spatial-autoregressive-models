...
N_repeat = 500;
dim = 1 + L + 3 + K;
pe_bgl_array = zeros(N_repeat, dim);
pm_bgl_array = cell(N_repeat, 1);
coverage_bgl = zeros(N_repeat, dim);
pspro_bgl = zeros(N_repeat, dim);
...
parfor loop = 1:N_repeat
    ...
    bgl_param = param;
    bgl_param.phi_g = [0, 0]';
    bgl_param.omega_sq = ones(G, 1);
    param_array_bgl = mcmc_bagl_sa(N_iter, N_init, N_burn, bgl_param, data, beta0, B0, c0, C0, alpha0, Alpha0);

    % Evaluate the estimation performance
    [~, pm_bgl, ~, ~, pe_bgl] = evaluate_simulation(true_param, param_array_bgl, 0.2);
    pe_bgl_array(loop, :) = pe_bgl;
    pm_bgl_array{loop} = pm_bgl;

    for i = 1:dim
        bound_bgl = hpdi(pm_bgl(:, i), 0.95);

        if (bound_bgl(2) > true_param_array(1, i)) && (bound_bgl(1) < true_param_array(1, i))
            coverage_bgl(loop, i) = 1;
        end

        if (bound_bgl(1) > 0) || (bound_bgl(2) < 0)
            pspro_bgl(loop, i) = 1;
        end
...
mean_bgl = mean(pe_bgl_array);
std_bgl = std(pe_bgl_array);
bias_bgl = mean(pe_bgl_array) - true_param_array;
rmse_bgl = sqrt(mean((pe_bgl_array - true_param_array) .^ 2));
cover_bgl = sum(coverage_bgl) / N_repeat;
psp_bgl = sum(pspro_bgl) / N_repeat;

estimation_result = table(mean_bgl', std_bgl', bias_bgl', rmse_bgl', cover_bgl', psp_bgl', ...
    'VariableNames', ...
    {'mean_bgl', 'std_bgl', 'bias_bgl', 'rmse_bgl', 'cover_bgl', 'psp_bgl'});

writetable(estimation_result, 'simu_bgl_dgp2.csv')
save('simu_bgl_dgp2.mat');
