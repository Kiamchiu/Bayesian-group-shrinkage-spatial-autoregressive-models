...
true_lambda = 0.4;
true_gamma = [0.5; 0.0; 0.0; 0.50; 0.00; 0.00; 0.00; 0.00];
...
N_repeat = 500;
dim = 1 + L + 3 + K;
pe_basgl_array = zeros(N_repeat, dim);
pm_basgl_array = cell(N_repeat, 1);
coverage_basgl = zeros(N_repeat, dim);
pspro_basgl = zeros(N_repeat, dim);
...
parfor loop = 1:N_repeat
    ...
    basgl_param.phi_1_sq = 1;
    basgl_param.phi_2_g = ones(G, 1);
    basgl_param.tau_sq_inv = zeros(L, 1);
    basgl_param.nu_sq_inv = ones(G, 1);

    param_array_basgl = mcmc_basgl_sa_b(N_iter, N_init, N_burn, basgl_param, data, beta0, B0, c0, C0, alpha0, Alpha0, d1);

    % Evaluate the estimation performance
    [~, pm_basgl, ~, ~, pe_basgl] = evaluate_simulation(true_param, param_array_basgl, 0.2);
    pe_basgl_array(loop, :) = pe_basgl;
    pm_basgl_array{loop} = pm_basgl;

    for i = 1:dim
        bound_basgl = hpdi(pm_basgl(:, i), 0.95);

        if (bound_basgl(2) > true_param_array(1, i)) && (bound_basgl(1) < true_param_array(1, i))
            coverage_basgl(loop, i) = 1;
        end

        if (bound_basgl(1) > 0) || (bound_basgl(2) < 0)
            pspro_basgl(loop, i) = 1;
        end
...
mean_basgl = mean(pe_basgl_array);
std_basgl = std(pe_basgl_array);
bias_basgl = mean(pe_basgl_array) - true_param_array;
rmse_basgl = sqrt(mean((pe_basgl_array - true_param_array) .^ 2));
cover_basgl = sum(coverage_basgl) / N_repeat;
psp_basgl = sum(pspro_basgl) / N_repeat;

estimation_result = table(mean_basgl', std_basgl', bias_basgl', rmse_basgl', cover_basgl', psp_basgl', ...
    'VariableNames', ...
    {'mean_basgl', 'std_basgl', 'bias_basgl', 'rmse_basgl', 'cover_basgl', 'psp_basgl'});

writetable(estimation_result, 'simu_basgl_dgp2.csv')
save('simu_basgl_dgp2.mat');