...
true_lambda = 0.4;
true_gamma = [0.5; 0.0; 0.0; 0.5; 0.0; 0.00; 0.00; 0.00];
...
N_repeat = 500;
dim = 1 + L + 3 + K;
pe_up_array = zeros(N_repeat, dim);
pm_up_array = cell(N_repeat, 1);
coverage_up = zeros(N_repeat, dim);
pspro_up = zeros(N_repeat, dim);
...
parfor loop = 1:N_repeat
    ...
    up_param = param;
    param_array_up = mcmc_up(N_iter, N_init, N_burn, up_param, data, beta0, B0, a0, b0, c0, C0, alpha0, Alpha0);

    % Evaluate the estimation performance

    [~, pm_up, ~, ~, pe_up] = evaluate_simulation(true_param, param_array_up, 0.2);

    pe_up_array(loop, :) = pe_up;
    pm_up_array{loop} = pm_up;

    for i = 1:dim
        bound_up = hpdi(pm_up(:, i), 0.95);

        if (bound_up(2) > true_param_array(1, i)) && (bound_up(1) < true_param_array(1, i))
            coverage_up(loop, i) = 1;
        end

        if (bound_up(1) > 0) || (bound_up(2) < 0)
            pspro_up(loop, i) = 1;
        end
...
mean_up = mean(pe_up_array);
std_up = std(pe_up_array);
bias_up = mean(pe_up_array) - true_param_array;
rmse_up = sqrt(mean((pe_up_array - true_param_array) .^ 2));
cover_up = sum(coverage_up) / N_repeat;
psp_up = sum(pspro_up) / N_repeat;

estimation_result = table(mean_up', std_up', bias_up', rmse_up', cover_up', psp_up', ...
    'VariableNames', ...
    {'mean_up', 'std_up', 'bias_up', 'rmse_up', 'cover_up', 'psp_up'});

writetable(estimation_result, 'simu_up_dgp2.csv')
save('simu_up_dgp2.mat');
